...
export async function existingBuiltinKinds(userId: string): Promise<Set<string>> {
  const rows = await prisma.ledger.findMany({
    where: { userId, deletedAt: null, kind: { in: ['work', 'taoyuan'] } },
    select: { kind: true },
  });
...
export async function applyImport(
  ...
  await prisma.$transaction(
    async (tx) => {
      if (mode === 'replace') {
        // 顺序同样要照顾外键：先删子表再删父表。
        // Ledger 的级联会带走 GeneralEntry / TripMember / TripExpense / TripSplit，
        // 但显式删一遍更好读，也不依赖 schema 里的 onDelete 配置不被改动。
        await tx.tripSplit.deleteMany({ where: { expense: { ledger: { userId } } } });
        await tx.tripExpense.deleteMany({ where: { ledger: { userId } } });
        await tx.tripMember.deleteMany({ where: { ledger: { userId } } });
        await tx.generalEntry.deleteMany({ where: { ledger: { userId } } });
        await tx.eventAmount.deleteMany({ where: { event: { userId } } });
        // 先摘掉父子关系再删，避免自引用外键的删除顺序问题
        await tx.event.updateMany({ where: { userId }, data: { parentId: null } });
        await tx.event.deleteMany({ where: { userId } });
        await tx.entry.deleteMany({ where: { userId } });
        await tx.ledger.deleteMany({ where: { userId } });
      }

      if (plan.ledgers.length) await tx.ledger.createMany({ data: plan.ledgers });
