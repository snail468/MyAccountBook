...
| 单元测试 | ✅ 329 个，全部通过（route 层不在单测范围，靠 build + 真实请求冒烟覆盖） |
...
| **B8 离线记账队列** | 已经是 PWA，但 `public/sw.js` 只缓存静态资源，断网完全不能记。用 IndexedDB 暂存 + 联网后补交。**上一轮延后原因**：需 Service Worker + IndexedDB + 真实断网验证，无浏览器/CI 覆盖 | 中 |
| **B9 账单导入** | 支付宝 / 微信账单 CSV 解析导入，比手动记账省事一个数量级 | 中 |
| ~~B10 预算升级~~ | ✅ 分类别预算已完成，见 [2.23](#223-四项迭代垫款--京东卡--批量回款--分类预算)。**剩余**：周预算、超支 Web Push 提醒 | 低 |
| ~~B6 账户/资产概念~~ | 你未选，跳过 | — |
...
```bash
npm run verify        # typecheck + lint + 329 个单测，提交前跑
npm test              # 只跑单测
