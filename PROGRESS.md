...
| 单元测试 | ✅ 146 个，全部通过（route 层不在单测范围，靠 build + 真实请求冒烟覆盖） |
...
| 项 | 说明 |
|---|---|
| 上传前图片压缩 | 现在单张允许 8MB 原图，显示时用原图 `<img>` 塞进 32×32 缩略框。手机传几张原图，列表页要下载几十 MB。建议上传前用 canvas 压缩，或生成缩略图变体 |
...
| **搜索与筛选** | 目前所有列表只能肉眼翻。按金额区间、类别、标签、时间范围、备注关键字筛选，跨账本全局搜索 |
| **统计图表页** | 数据都有了但只展示了「本月 Top5 类别」。月度收支趋势折线、类别占比环形图、同比/环比、年度报告（可沿用已有 `TripFunReport` 的风格） |
...
- 新增 `BankCard` 模型：银行名 / 卡别名 / 卡类型 / 持卡人 / 尾号明文
- 完整卡号与备注用 AES-GCM 加密落库，密钥来自 `CARD_SECRET` 环境变量（`src/lib/env.ts` 里已预留 `getCardSecret()`，目前无人调用）
- **绝不存 CVV 与密码**
- UI 默认打码，查看完整需二次输入登录密码
- 导出时默认不含卡号，需显式勾选
...
```bash
npm run verify        # typecheck + lint + 146 个单测，提交前跑
npm test              # 只跑单测
