...
| 单元测试 | ✅ 308 个，全部通过（route 层不在单测范围，靠 build + 真实请求冒烟覆盖） |
...
| ~~C13 工作账本~~ | ✅ 按类别累计统计卡片（见 2.6）+ **未回款超期提醒**（见 [2.22](#222-未回款超期提醒--类别智能排序)）。**剩余**：按类别趋势（月度分类走势） |
...
```bash
npm run verify        # typecheck + lint + 308 个单测，提交前跑
npm test              # 只跑单测
